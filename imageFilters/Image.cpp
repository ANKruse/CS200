...
void Image::ReadFile( const string& filename )
{
    ...
    {
        infile >> pixelArray[pixel].r;
        infile >> pixelArray[pixel].g;
        infile >> pixelArray[pixel].b;
    }
    infile.close();
}
...
void Image::WriteFile( const string& filename )
{
    ...
    for (int pixel = 0; pixel < pixelCount; pixel++)
    {
        output << pixelArray[pixel].r << " " << pixelArray[pixel].g << " "
        << pixelArray[pixel].b << " " << endl;
        cout << pixelArray[pixel].r << " " << pixelArray[pixel].g << " "
        << pixelArray[pixel].b << " " << endl;
    }
    output.close();
}
...
void Image::Filter_Brighten()
{
    for ( int pixel = 0; pixel < pixelCount; pixel++)
    {
        pixelArray[pixel].r = (2 * pixelArray[pixel].r);
        if (pixelArray[pixel].r > 255)
        {
            pixelArray[pixel].r = 255;
        }
        pixelArray[pixel].g = (2 * pixelArray[pixel].g);
        if (pixelArray[pixel].g > 255)
        {
            pixelArray[pixel].g = 255;
        }
        pixelArray[pixel].b = (2 * pixelArray[pixel].b);
...
void Image::Filter_ShiftColors()
{
    for ( int pixel = 0; pixel < pixelCount; pixel++)
    {
        ...
        pixelArray[pixel].b = (pixelArray[pixel].r);
    }
}
...
void Image::Filter_Custom()
{
    for ( int pixel = 0; pixel < pixelCount; pixel++)
    {
        pixelArray[pixel].r = (255 - pixelArray[pixel].r);
        pixelArray[pixel].g = (255 - pixelArray[pixel].g);
        pixelArray[pixel].b = (255 - pixelArray[pixel].b);
    }
}
